...
export const Navbar: React.FC = () => {
  ...
  return (
    <>
      <header className="fixed top-0 left-0 right-0 z-40 w-full bg-white/95 backdrop-blur-xl border-b border-[#E8DFD1] shadow-xs">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            >
              <div className="flex items-center justify-center w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500/15 to-violet-500/15 border border-emerald-500/30 shadow-xs group-hover:scale-105 transition-transform">
                <Sparkles className="w-5 h-5 text-emerald-600 animate-pulse" />
              </div>
              <div>
                <div className="flex items-center space-x-2">
                  <span className="font-heading font-black text-lg tracking-tight text-[#1C1917]">
                    Waste<span className="text-emerald-600">2</span>Menu
                  </span>
                  {isAuthenticated ? (
                    <span className="px-1.5 py-0.5 text-[9px] font-mono font-bold uppercase tracking-wider bg-emerald-100 text-emerald-800 border border-emerald-200 rounded">
                      {userPersona === 'chef' ? 'Chef' : 'Home'}
                    </span>
                  ) : (
                    <span className="px-1.5 py-0.5 text-[9px] font-mono font-bold uppercase tracking-wider bg-stone-100 text-stone-600 border border-stone-200 rounded">
                      Public
                    </span>
                  )}
                </div>
                <div className="flex items-center space-x-1.5 text-[10px] text-[#6B6358]">
                  <span className="flex h-1.5 w-1.5 relative">
                    <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-500 opacity-75"></span>
                    <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-emerald-600"></span>
                  </span>
                  <span className="font-mono text-emerald-700 font-semibold">LIVE SYNC</span>
            ...
            {/* Center: Dynamic Tabs */}
            {/* CASE 1: OUTSIDE DASHBOARD (LOGGED OUT) -> Shows Public Sections */}
            {!isAuthenticated ? (
              <nav className="hidden lg:flex items-center space-x-1 bg-[#F4EFEA] p-1 rounded-2xl border border-[#E8DFD1]">
              ...
              /* CASE 2: LOGGED IN -> Shows Station Tabs According to Role */
              <nav className="hidden lg:flex items-center space-x-1 bg-[#F4EFEA] p-1 rounded-2xl border border-[#E8DFD1]">
            ...
            <div className="flex items-center space-x-2 sm:space-x-2.5">
              ...
              {isAuthenticated ? (
                <button
                  ...
                  className="flex items-center space-x-2 p-1.5 pr-2.5 rounded-xl bg-white hover:bg-stone-50 border border-[#E8DFD1] transition-all shadow-xs"
                >
                  <img
                  ...
                  <div className="hidden sm:block text-left leading-tight">
                    <div className="text-xs font-heading font-bold text-stone-900 truncate max-w-[110px]">
                      {userProfile?.name}
                    </div>
                ...
                <div className="flex items-center space-x-1.5">
                  <button
                    ...
                    className="flex items-center space-x-1.5 px-3 py-1.5 rounded-xl bg-white hover:bg-stone-50 border border-[#E8DFD1] text-stone-800 font-heading font-bold text-xs shadow-xs transition-all"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center space-x-1.5 px-3 py-1.5 rounded-xl bg-emerald-700 hover:bg-emerald-800 text-white font-heading font-bold text-xs shadow-xs transition-all"
                  >
              ...
              <button
                ...
                className="p-2.5 rounded-xl bg-white hover:bg-[#F5EFEB] text-[#6B6358] hover:text-[#1C1917] border border-[#E8DFD1] transition-colors shadow-xs"
              >
                {soundEnabled ? (
              ...
              <button
                ...
                className="relative p-2.5 rounded-xl bg-white hover:bg-[#F5EFEB] text-[#6B6358] hover:text-[#1C1917] border border-[#E8DFD1] transition-colors shadow-xs"
                title="System Notifications"
              >
                <Bell className="w-4 h-4" />
                {unreadCount > 0 && (
                  <span className="absolute -top-1 -right-1 flex items-center justify-center min-w-[18px] h-[18px] px-1 text-[10px] font-bold font-mono bg-rose-600 text-white rounded-full border-2 border-white shadow-xs animate-pulse">
                    {unreadCount}
                  </span>
                )}
              </button>

              {/* Mobile Role / Section Switcher Select */}
              <div className="lg:hidden">
                {!isAuthenticated ? (
                  <select
                    value={activeLandingSection}
                    onChange={(e) => setActiveLandingSection(e.target.value)}
                    className="bg-white text-[#1C1917] text-xs font-semibold py-2 px-2.5 rounded-xl border border-[#E8DFD1] shadow-xs focus:outline-none focus:border-emerald-500"
                  >
      ...
      {/* Active User Profile & Sign Out Drawer Modal (Isolated Session) */}
      {isProfileModalOpen && isAuthenticated && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-stone-900/50 backdrop-blur-sm flex justify-center items-start p-4 pt-24 sm:pt-28 pb-12">
          <div className="bg-[#FFFDF9] border border-[#E8DFD1] rounded-3xl max-w-md w-full p-6 shadow-2xl relative space-y-5">
            ...
            {/* Security Isolation Notice (Explaining why another account cannot be accessed while logged in) */}
